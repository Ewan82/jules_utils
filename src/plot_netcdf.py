...
def plot_ghana(f_name, var_name, t_step, layer=0, save_fig=0, fig_path='None', img_res=70, mask_ocean=True,
    ...
    # draw color mesh.
    cs = m.pcolormesh(x, y, var_mask, cmap='YlGnBu', vmax=v_max, vmin=0,)
    # add colorbar.
...
def save_many_ghana(f_name, var_name, t_step_list, layer=0, fig_path='None', colormap='YlGnBu', vmax='None'): #'Blues' for rain?
    ...
    #maks oceans for requested var, return x, y coords for basemap instance
    if vmax == 'None':
        cs = m.imshow(var_slice[0], cmap=colormap,
